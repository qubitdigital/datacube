package com.urbanairship.datacube.dbharnesses.bigtable;
...
public class BigTableDbHarness<T extends Op> implements DbHarness<T> {
    ...
    public Optional<T> get(Address c) throws IOException, InterruptedException {
        ...
        Result result = WithBigTable.get(connection, tableName, get);
    ...
    private long increment(byte[] rowKey, T op) throws IOException {
        ...
        return WithBigTable.increment(connection, tableName, rowKey, cf, QUALIFIER, amount);
    }
    ...
    private byte[] readCombineCas(byte[] rowKey, T newOp) throws IOException {
        ...
        for (int i = 0; i < numCasTries; i++) {
            ...
            if (WithBigTable.checkAndPut(connection, tableName, rowKey, cf, QUALIFIER, prevSerializedOp, put)) {
                casTries.update(i + 1);
    ...
    private void overwrite(byte[] rowKey, T op) throws IOException {
        ...
        WithBigTable.put(connection, tableName, put);
        if (log.isDebugEnabled()) {
    ...
    public List<Optional<T>> multiGet(List<Address> addresses) throws IOException {
        ...
        final Result[] results = WithBigTable.get(connection, tableName, gets);
